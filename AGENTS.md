...
- 2026-09-25 — Inventory sprite fix on `fix/inventory-item-sprites`: bag item IDs require save context before asset lookup (FR Moon Stone 94 maps to modern 81). Consumer resolver reuses invariant item identities, legacy Core conversions, generic TM/TR sprites, and existing artwork; unmatched items use the unknown icon. Gen 1 catch-rate conversion is not a bag mapping. GameCube item IDs from 500 are game-specific. Held overlays use `SpriteItem` and bound artwork size. Regression tests cover identity pixels, raw-ID save/reset preservation, and an opt-in FireRed inventory capture.
- Validation: Release build 0 warnings/errors, 62 focused tests passed, FireRed Skia capture reviewed. Core 449 passed/1 skip; architecture 6 passed. Full Avalonia run had 4 failures: three layout/geometry failures reproduce on clean baseline `d0795e0a2`; the ColorPicker assembly failure in `LanguageMenuUsesNativeRadioItemsAndChangesLanguage` passed on isolated rerun and in the baseline full suite. Core mutates cached item strings for display, so the resolver reads pristine embedded text with `Util.GetStringResource`. Work remains local.
