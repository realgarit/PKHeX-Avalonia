...
- 2026-09-26 — Active ten-issue wave on `codex/issue-wave-september` in `C:\Git\PKHeX-issue-wave`: #225, #236–#239, #297, #307, #308, #324, #329, plus the requested top-row workspace navigation. Consumer changes add modeless Encounter Database session ownership, interface-gated Pokémon metadata and separate Plus flags, confirmed X/Y Friend Safari unlocks, transactional Beans/Cells/Records/Poffins, and named BDSP Poffin types. Gen 5 record reads decrypt data, so inspection uses a clone and commit ends encrypted access; preserve existing out-of-range values unless edited. SM Stored cells are not available map cells: counter deltas preserve spent cells and core counts. Tests and opt-in Skia captures are under active validation (`IssueWaveViewTests`, `tmp/issue-wave-captures`). Native ZA metadata/Plus views were inspected; desktop control was interrupted with Escape before the remaining native cases. Shipping remains outstanding. Core/AutoMod and CI-owned versions are untouched.
