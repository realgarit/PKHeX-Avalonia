...
- 2026-09-26 — Active ten-issue wave on `codex/issue-wave-september` in `C:\Git\PKHeX-issue-wave`: #225, #236–#239, #297, #307, #308, #324, #329, plus the requested top-row workspace navigation. Consumer changes add modeless Encounter Database session ownership, interface-gated Pokémon metadata and separate Plus flags, confirmed X/Y Friend Safari unlocks, transactional Beans/Cells/Records/Poffins, and named BDSP Poffin types. Gen 5 record reads decrypt data, so inspection uses a clone and commit ends encrypted access; preserve existing out-of-range values unless edited. SM Stored cells are not available map cells: counter deltas preserve spent cells and core counts. Release build: 0 warnings/errors; full suite: Avalonia 2,795 passed/1 existing skip, Core 449/1 existing skip, Architecture 6/0. Reviewed Skia captures are in `tmp/issue-wave-captures`; the README dark/light lead PNGs now use the ZA fixture with every displayed entity verified legal. Draft PR #400 is open. Native Windows QA covered ZA metadata/Plus flags, 95-cell SM and 100-sticker USUM sessions, real Gen5/6/7/SWSH/BDSP Records, upstream-comparable BDSP Poffin fields/presets, and modeless Encounter Database selection, reuse, main-window interactivity, and save-close cleanup. X/Y Friend Safari confirmation cancel/apply paths were exercised. USUM accessibility names now identify Sticker counters. Final Release build is 0 warnings/errors and full suite is Avalonia 2,797 passed/1 existing skip, Core 449/1 existing skip, Architecture 6/0. Skia frames include issue-wave views and the legal-fixture README lead images. The remaining gates are the final PR checks, merge/release, and cleanup. Core/AutoMod and CI-owned versions are untouched. Core/AutoMod and CI-owned versions are untouched.

- 2026-09-20 — README gallery follow-up: keep a centered 600px hero preview; nine app images are grouped in five initially collapsed GitHub details sections. Seven additional frames cover Trainer, Inventory, Reports, tool launcher, BDSP Pokédex/Seal Stickers, and Technical Records, generated headlessly from public/synthetic fixtures. Capture provenance is in `docs/screenshots/README.md`.

- 2026-09-20 — README refresh on `docs/refresh-readme-screenshots`: current navigation, verified v1.49.12 artifact names, cautious legality/generation descriptions, quick-start slot Set workflow, complete project dependencies, and AI-assisted contribution guidance. New 900x600 dark/light screenshots come from the existing Skia headless capture test with synthetic entities; reproduction and provenance are in `docs/screenshots/README.md`. Documentation-only changes do not require a release bump.
...
- 2026-09-21 - Issue #396 triage on codex/fix-linux-save-open-crash: Linux/AppImage save-open could raise SaveFileChanged from a native file-picker continuation's worker thread, causing Avalonia's `Call from invalid thread` while CurrentSave notified bound MenuItems. MainWindowViewModel now marshals the whole save-adoption path through IUiDispatcher; the worker-thread regression test fails on the old callback and passes with the fix. Release build is 0-warning/0-error; full gates are Avalonia 2,734 passed/1 existing skip, Core 449 passed/1 existing skip, Architecture 6/0. Native AppImage reproduction remains pending because this Windows session lacks WSL and is non-administrator.
