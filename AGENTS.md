...
- 2026-09-26 — Ten-issue implementation on `codex/issue-wave-september`, draft PR #400: #225, #236–#239, #297, #307, #308, #324, and #329. Added capability-gated Pokémon metadata and a separate Plus Move editor; modeless Encounter Database; confirmed X/Y Friend Safari unlock; transactional Beans, Cells/Stickers, Records, and Poffins; named BDSP Poffin fields; direct top-row Pokémon/Trainer/Inventory/Save/Reports navigation; and legal-fixture 900×600 dark/light README lead captures. Gen 5 records are read from a clone and ended before export; unchanged out-of-range values are preserved. SM Stored cells are not available map cells, so staged count changes preserve spent/cored cells. Release build has 0 warnings/errors; Avalonia 2,797 passed/1 existing skip, Core 449/1 existing skip, Architecture 6/0. Native QA covered ZA metadata/Plus, SM and USUM transaction dialogs and screen-reader names, all five Records formats plus invalid input/search and native maximize/reopen size retention, upstream PKHeX BDSP Poffin comparison, X/Y Friend Safari Cancel/Apply, and modeless encounter selection/reuse/save-close. Core/AutoMod and CI-owned versions are untouched. Current PR head is `10636c657`; final build and CI, merge/release verification, Discord shipped update, and worktree cleanup remain.
